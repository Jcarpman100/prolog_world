...
wumpus(X, Y) :- not(visited(X, Y)), X1 is X - 1, X2 is X + 1, Y1 is Y - 1, Y2 is Y + 1,
not(noStench(X1, Y)), not(noStench(X2, Y)), not(noStench(X, Y1)), not(noStench(X, Y2)).


% Possible Pit?
% if none of the neighboring spaces lack a breeze, we must assume a wumpus.
pit(X, Y) :- not(visited(X, Y)), X1 is X - 1, X2 is X + 1, Y1 is Y - 1, Y2 is Y + 1,
not(noBreeze(X1, Y)), not(noBreeze(X2, Y)), not(noBreeze(X, Y1)), not(noBreeze(X, Y2)).
...
% If the agent has the gold and has attempted to kill the wumpus, it may go home.
...
run_agent([yes,_,_,_,_], shoot):- haveArrow, location(X,Y), next_tile(X, Y, Dx, Dy), conWumpus(Dx,Dy),
write("The wumpus is in my sight!.\n"), assert(safety(Dx, Dy)), retract(haveArrow).


% Murder
% If there is a known wumpus location is nearby, turn to commit murder.
run_agent(_,turnright):- haveArrow, location(X,Y), wumpusFound(X,Y), replace_existing_fact(boredom(G),
boredom(0)), direction(A), turn(A,B), replace_existing_fact(direction(A), direction(B)), write("I found the wumpus!").
...
run_agent([yes,_,_,_,_], shoot):- location(0,0), haveArrow, next_tile(0, 0, Dx, Dy), assert(safety(Dx, Dy)),
...
run_agent([_,_,_,_,_], turnright):- location(X,Y), next_tile(X, Y, Dx, Dy), not(murder), not(safety(Dx,Dy)),
wumpus(Dx, Dy), write("A wumpus is ahead.\n"), direction(A), turn(A,B), replace_existing_fact(direction(A), direction(B)).


% Avoid known walls
% If there is a wall ahead, turn right
run_agent(_,turnright):- location(X,Y), next_tile(X, Y, Dx, Dy), wall(Dx, Dy), direction(A), turn(A,B),
replace_existing_fact(direction(A), direction(B)), write("Wall ahead.").
...
run_agent(_,goforward):- location(X,Y), next_tile(X, Y, Dx, Dy),assert(visited(Dx, Dy)),
replace_existing_fact(location(X,Y), location(Dx, Dy)), write("forward!").
